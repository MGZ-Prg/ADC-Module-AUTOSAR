...
void Adc_Handler(Adc_GroupType Group)
{
	...
	/*Set the status of Adc Group to ADC_COMPLETED if : 
	 *If conversion round (not the final one) of the requested group has been finished
	*/
	
	/*Set the status of Adc Group to ADC_STREAM_COMPLETED if :
	 *If single access mode after one conversion round is completed.
   *If streaming access mode after the number of conversion rounds of the requested group have been finished, to fill the streaming buffer completely.
	*/
